...
const run = async lastNotice => {
  const [
    ...
    $.db.getExtConfig('notices.interval', '10m'),
    $.db.getExtConfig('notices.userLimit', 10),
    $.db.getExtConfig('notices.chatLines', 15),
    $.db.getExtConfig('notices.onlineOnly', true),
    $.db.find('notices')
  ])
...
export default {
  ...
  async setup (context) {
    ...
    const interval = await $.db.getExtConfig('notices.interval', '10m')
    if (interval !== '-1s') {
